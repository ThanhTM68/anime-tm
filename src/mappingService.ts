 ...
 */
export function generateAni4uLink(anime: Anime, episode?: number | null): LinkResult {
...
export function getAnimeVietSubUrl(anime: Anime, episode?: number): string {
  ...
  return `https://animevietsub.show/tim-kiem/${encodeURIComponent(anime.title)}/`;
}
...
export function getTrailerUrl(anime: Anime): string {
  return anime.trailerUrl || `https://www.youtube.com/results?search_query=${encodeURIComponent(anime.title + ' trailer')}`;
}
